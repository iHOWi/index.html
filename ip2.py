...
TOKEN = "8009426826:AAHgd0hmxREuMxSnm2HDkyyF62CIRVRtrlk"
CHANNEL_ID = "@ip_searcher"
...
async def handle_insults(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем, что сообщение и текст существуют
...
async def check_subscription(user_id: int, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """
    Checks if a user is subscribed to the specified channel with caching.
    """
    # Проверяем кэш (актуальность 5 минут)
    ...
    try:
        ...
        subscription_cache[user_id] = (datetime.now(), result)
        return result
    except Exception:
        subscription_cache[user_id] = (datetime.now(), False)
        return False
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Можно добавить логирование ошибок в файл или отправку администратору
